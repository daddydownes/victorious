...
## Late initial page-load race

The first live release `ee30925` passed desktop stress and browser history, but [one live portrait touch run](initial-live-touch-failure.json) still cancelled an entry. Investigation found that the controller treated an ordinary initial `pageshow` like a cached-page restoration. Users can begin interacting after DOMContentLoaded and child readiness while other resources still delay initial load/pageshow.

A controlled real-browser experiment delayed initial load with one test-only invisible image, leaving product JavaScript unchanged. Releasing it during an entry caused real `load` and `pageshow` (`persisted:false`), immediately freezing the old controller at a partial viewport. [Before](late-pageshow-before.json) and [after](late-pageshow-after.json) isolate the cause. Source `49fa453` (`ee5dea0` before integration) restricts cancellation/deactivation/synchronization to persisted restoration; an initial pageshow simply requests measurement. The controlled case now completes, and [portrait/landscape touch stress](touch-final-local.json) remains uninterrupted. The first live failure had no event log, so the controlled experiment establishes the real race without claiming that its exact triggering event was captured in that original run.

## Final published verification

Application release `255fb03a01bdc80762e3f0c2ad1b3117f48cefff` is built and live at https://vctrsclo.com. [Seven canonical resources](canonical-live.json) match the local release. The final story SHA-256 is `cdc4f336d94e45633fd3ec14ac41e73a056018d03ae04bc03353889867834f26`.

[First fresh live touch run](touch-live-1.json) and [second fresh live touch run](touch-live-2.json) each cover portrait and landscape: all four cases capture that response fingerprint and show one entry with zero interruptions. Reverse/retry and child panning controls pass. An additional [fingerprinted diagnostic](diagnostic-live-summary.json) records zero interruptions after commitment and no page errors; initial inactive samples before the initiating gesture are excluded from that post-commit metric. The [real delayed initial pageshow fixture](late-pageshow-live.json) also passes against live product JavaScript. These are browser touch-emulation checks, not physical-device certification.

Final source validation: navigation VM 13/13, deterministic integration 15/15, protected game-source checks, media identity 37/37, and eleven local browser refresh/history cases pass. Original HD assets remain byte-identical.
