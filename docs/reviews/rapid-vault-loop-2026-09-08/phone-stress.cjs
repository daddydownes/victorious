...
const out = process.env.AUDIT_OUT || '/private/tmp/vctrs-fast-touch-after';
...
(async () => {
  const specs = baselineOnly
    ...
    : [
        ...
        { engine: 'chromium', width: 390, height: 844, name: 'chromium-390x844', loops: 5, touch: true },
        { engine: 'chromium', width: 844, height: 390, name: 'chromium-844x390', loops: 5, touch: true }
      ];
  const results = [];
