...
async function fixture(page){
  ...
  return page.evaluate(()=>{
    const game=__flap.dbg(),base=game.gates[0],w=720*innerWidth/innerHeight,serials=[3,4,5],phases=[.2,2.3,5.1],xs=[.42,.64,.86];
...
async function run(width,height,touch){
  ...
  try{
    ...
    await page.screenshot({path:path.join(out,`${name}-serials-3-4-5-before.png`)});
    ...
    await page.screenshot({path:path.join(out,`${name}-serials-3-4-5-after-shift.png`)});
    assert.deepEqual(after.gates.map(g=>[g.serial,g.phase,g.kind]),before.gates.map(g=>[g.serial,g.phase,g.kind]));
    assert(before.gates.every((g,i)=>g.serial===i+3&&g.phase===[.2,2.3,5.1][i]&&g.kind==='PILLAR'));assert.equal(before.overflow,false);assert.equal(after.overflow,false);
