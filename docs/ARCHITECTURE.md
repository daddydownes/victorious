...
`tools/guided/collection.html` contains the unchanged Previous Drops, signup and Vault invitation. The pre-Vault controller in `tools/guided/journey.js` owns accepted wheel, single-touch and navigation-key input for this panel. It moves the existing scroll position between collection, signup, readable intermediate stops for tall signup content, overview and the zoom endpoint. Each bounded eased transition coalesces additional forward input; accumulated reverse intent redirects it. `window.__vaultCamera.paint()` positions the original root `#vault` photo plane from that scroll position, and `commitVaultCamera()` hands off its exact final frame without a second photo grid. The opening hero, archive input and all post-Surface controllers are independent and unchanged.

The bird's-eye scene begins with all 33 photos inside the phone or desktop viewport, then scales the same plane toward its interactive identity transform over the existing `200svh` section. The full overview is a separate landing before the one-viewport zoom. Camera progress is continuous; transition completion snaps the final sub-three-pixel easing tail to its exact target so a fresh gesture is not swallowed by invisible remaining motion. `warmOverview()` requests the 640px derivatives as the visitor nears the section; the four-request queue prioritizes nearby upgrades, while the Vault readiness promise covers only the landing area. The baked `PHOTOS` arrangement and root camera are unchanged by the guided input correction. `tests/vault-spatial-layout.cjs` checks the arrangement; `tests/entry-scroll-browser.cjs` checks full real-input journeys and records candidate hashes.
...
The controller enters only after a deliberate forward transition reaches the exact endpoint, all touch contacts end and wheel input becomes quiet. Cancellation, multiple contacts, resize, blur and hidden-page cleanup revoke pending entry, including permission held by an unfinished transition. A later deliberate gesture recovers; geometry alone cannot enter. Still-view resize keeps the current named landing aligned to new geometry. Reduced motion uses immediate landings with the same release gate. `window.__vaultEntryGuide` exposes read-only transition state for diagnostics.
...
The labelled #collectionScrollCue and #vaultScrollCue use the same guided transition as gestures and focus their target heading without opening the keyboard. Mandatory CSS snapping is disabled in this panel so it cannot compete with guided movement. While a reported keyboard viewport is active, native form scrolling remains available. With the keyboard closed, onward wheel/touch/navigation-key input outside an editable field transfers focus from the form to its heading and resumes guidance; later navigation keys remain inside the panel. The separate signup viewport controller still defers layout recovery through an active JOIN press. Pinch and editable-field gestures remain browser-owned. Both centred labels retain the shared gold-to-ivory light pass and reduced-motion treatment.
