...
Vault entry bounds original-image downloads and decoding to four concurrent jobs, with an eight-second preview fallback for input readiness. The guided controller keeps the vault inert/busy through that bounded readiness and the start of the title fade, without a second unbounded DOM decode. The approved 03 identity flight stays centred for 1550ms and immediately expands/fades over 1250ms. Its CSS lives in `tools/guided/journey.css`; the transfer and readiness lifecycle remain in the root. `__vaultInteractiveReady` represents the earlier input handoff; `__vaultTitleReady` tracks cosmetic completion. Global capture guards reject navigation input during opening/settling; native pan activates only once the inert gate clears. Reduced motion skips expansion. The opening film waits for actual playback completion during buffering and exposes an explicit Continue escape after 15 seconds without progress. The second film loads on section entry and exposes Retry after sustained buffering. See the [network audit](slow-network-audit.md) and historical [1080p review](reviews/original-1080-2026-09-09/README.md).
