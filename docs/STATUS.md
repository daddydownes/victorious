...
The local-only `demo/v-style-20260908` branch adds a clean V-style art direction to the story invitation and Flappy V. `Play the game`, `Back to the vault` and the play arrow are self-contained vector assets in the original hot gold (`#f0d492`), with flared serif forms, a few rounded drips and restrained edge flecks. Flappy obstacles retain their exact collision polygons and receive clipped V-and-star paint, a quiet dark face and a faint baked backdrop mark. The canonical V-and-star path, gameplay physics, 25-clear playable progression, ten-clear automatic preview and 100-clear reward are unchanged. All prior media files remain untouched.
