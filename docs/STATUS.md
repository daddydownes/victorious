...
Open the review copy at `http://127.0.0.1:8922/experience/#play`; the full story starts at `http://127.0.0.1:8922/experience/`. The implementation checkpoint is `7a91555`; later local commits add test-only coverage and this record. Source/static checks passed, including 15 protected game functions across all three routes, deterministic generation and the 16 integration checks. A delayed-title phone check passed without layout shift and verified the complete vault reveal. The warmed three-second frame sample measured p95 17.5ms, max 17.7ms, no frame above 25ms and no long task. Native Safari verified the direct anchor, preview, game start/flap/exit focus, both new headings and the vault action. Independent review found no local-demo code blocker. The broader mobile interaction check was still running when this checkpoint was written. This branch has not been pushed or published; production remains `81ab867`.
