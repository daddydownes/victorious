...
Open the review copy at `http://127.0.0.1:8922/experience/#play`; the full story starts at `http://127.0.0.1:8922/experience/`. The implementation checkpoint is `7a91555`; later local commits add test-only coverage and this record. Source/static checks passed, including 15 protected game functions across all three routes, deterministic generation and the 16 integration checks. Native Safari verified the direct anchor, preview, game start/flap/exit focus, both new headings and the vault action. Independent review found no local-demo code blocker. Focused phone loading/layout, mobile interaction and frame sampling were still running when this checkpoint was written. This branch has not been pushed or published; production remains `81ab867`.
