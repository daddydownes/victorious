...
Frameless gold-touch preview ready for design review on `demo/button-press-feedback`, at `demos/button-press-feedback/`. Removed inner rectangular border and inset shadows; kept contact light and smooth release. Assistive clicks reset the contact origin. Two agents reviewed CSS/input; primary integrated and tested four Chrome environments. All four reached the vault with correct focus and no recorded errors. Three portable suites pass. See [sprint results](reviews/press-sprint-2026-09-06.md). Not deployed; full repeated/physical-device release QA remains separate.
