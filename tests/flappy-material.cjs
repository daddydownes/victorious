...
function spyContext(actions, images) {
  ...
  return {
    ...
    fill() { actions?.push('paint'); }, fillRect() { actions?.push('paint'); }, drawImage(...args) { actions?.push('paint');if(images&&args.length===9){const [a,b,c,d,e,f]=matrix,x=args[5],y=args[6],w=args[7],h=args[8],points=[[x,y],[x+w,y],[x,y+h],[x+w,y+h]].map(([px,py])=>[a*px+c*py+e,b*px+d*py+f]),xs=points.map(p=>p[0]),ys=points.map(p=>p[1]);images.push({alpha,clipped,atlas:args[0].__flappyGraffitiAtlas===true,sourceW:args[0].width,sourceH:args[0].height,sx:args[1],sy:args[2],sw:args[3],sh:args[4],x:Math.min(...xs),y:Math.min(...ys),w:Math.max(...xs)-Math.min(...xs),h:Math.max(...ys)-Math.min(...ys)})} },
...
const rendererSource=html.slice(begin + '/* FLAPPY METAL BEGIN */'.length, end);vm.runInContext(rendererSource, sandbox);
...
assert.equal(sandbox.FLAPPY_GRAFFITI_ART.gold,'#f0d492','graffiti atlas uses canonical V gold');
assert(!rendererSource.includes('function drawFacePaint'),'retired line swash renderer remains embedded');
assert(!actions.includes('fill:#f0d492')&&!actions.includes('stroke:#f0d492'),'retired gold line swash still paints obstacle faces');
...
draw(spyContext([],secondMarks),tallTop,{d:2,h:720,cssH:720},'pillar',0,.9);draw(spyContext([],secondMarks),tallBottom,{d:2,h:720,cssH:720},'pillar',1,.9);assert.deepEqual(secondMarks.filter(isGraffiti),graffiti,'paint placement changed with animation phase');assert.equal(randomCalls,0,'obstacle decoration consumed random state');
