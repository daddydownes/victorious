...
for(const file of ['index.html','experience/index.html','experience/game-preview.html']){const generated=read(file);assert(generated.includes("side,g.serial*16+(g.phase||0));"),file+' does not seed graffiti from immutable gate identity');assert(!generated.includes("side,(FG.bgT||0)*.08+g.serial*.13);"),file+' still animates the graffiti seed')}
