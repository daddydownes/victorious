...
const vaultSection=story.match(/<section\b[^>]*\bid="story-vault"[^>]*>/)?.[0]||'',vaultFrame=story.match(/<iframe id="story-vault-frame"[\s\S]*?<\/iframe>/)?.[0]||'';
assert(/\bclass="[^"]*\bstory-vault\b/.test(vaultSection),'embedded Vault section is missing');
for(const token of ['title="The VCTRS Vault"','loading="eager"','tabindex="-1"','inert','aria-hidden="true"'])assert(vaultFrame.includes(token),'embedded Vault frame is missing '+token);
const vaultCycle=vaultFrame.match(/data-src="\.\.\/vault-embed\.html\?embed=vault&amp;cycle=(\d+)"/)?.[1],parentCycle=story.match(/let active=false,loaded=false,requested=false,queued=false,resetting=false,cycle=(\d+)/)?.[1];assert(vaultCycle!==undefined&&vaultCycle===parentCycle,'iframe and parent controller disagree on the initial hashless cycle');assert(!vaultFrame.includes('#vault'),'embedded Vault URL still contains an ancestor-scrolling fragment');
...
assert(story.includes('requested=true;frame.contentWindow.location.replace(new URL(frame.dataset.src,location.href).href)'),'Vault preload is no longer history-neutral');
...
{const preview=read('experience/game-preview.html');assert(preview.includes("side,g.serial*16+(g.phase||0),false);"),'preview still decorates obstacle pairs');assert(!preview.includes("side,g.serial*16+(g.phase||0),true);")&&!preview.includes("side,g.serial*16+(g.phase||0),g.serial%5===4);"),'preview uses a decorated playable-game paint rule')}
