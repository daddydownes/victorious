...
(async()=>{for(const [engine,width,height] of [['chromium',1440,900],['webkit',390,844],['firefox',320,568]]){
 ...
 try{
  ...
  await page.waitForFunction(()=>vault.classList.contains('vault-previewing')&&Math.abs(parseFloat(vault.style.clipPath.slice(6))-nextDrop.clientHeight/2)<3);
  const entering=await page.evaluate(()=>({progress:Number(document.querySelector('.vault-descent-scene').style.getPropertyValue('--vault-progress')),top:vault.getBoundingClientRect().top,clip:parseFloat(vault.style.clipPath.slice(6)),transform:vault.style.transform}));
  assert.equal(entering.progress,0);assert(Math.abs(entering.top)<1);assert(Math.abs(entering.clip-height/2)<3);assert.equal(entering.transform,'');
 ...
 await page.waitForFunction(()=>Math.abs(vaultInvitation.getBoundingClientRect().top-nextDrop.getBoundingClientRect().top)<2&&vault.classList.contains('vault-previewing'));
  ...
  assert(start.travel>.98&&start.travel<1.02,'the overview-to-Vault zoom should take one screenful: '+JSON.stringify(start));
