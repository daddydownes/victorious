...
for(const name of ['failure.json','results.json'])fs.rmSync(path.join(out,name),{force:true});
...
async function arrivalAndRealPlay(browser){
 ...
 try{
  ...
  await page.evaluate(()=>{const canvas=document.getElementById('flap');window.__pointerWitness={};canvas.addEventListener('pointerdown',()=>{window.__pointerWitness.before={state:__flap.state(),vy:__flap.dbg().vy,kind:__flap.dbg().gates[0]?.kind}},{capture:true,once:true});canvas.addEventListener('pointerdown',()=>{window.__pointerWitness.after={state:__flap.state(),vy:__flap.dbg().vy}},{once:true})});
