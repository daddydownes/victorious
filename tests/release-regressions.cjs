...
async function pendingTitle(){
 ...
 try{
  ...
  assert.equal(complete.shiftTotal-pending.shiftBaseline,0,'pending art completion caused layout shift');await page.screenshot({path:path.join(out,'chromium-phone-play-title-ready.png')});
  await page.locator('#vault-invite').scrollIntoViewIfNeeded();await page.waitForFunction(()=>document.getElementById('vault-title').classList.contains('paint-pending'));await held('back-to-the-vault-vstyle-v1.svg');
  const vaultPending=await page.locator('#vault-title').evaluate(title=>{const r=title.getBoundingClientRect(),text=title.querySelector('.paint-title-text'),tr=text.getBoundingClientRect();return {rect:{x:r.x,y:r.y,width:r.width,height:r.height},textVisible:tr.width>20&&tr.height>20&&getComputedStyle(text).visibility!=='hidden',artVisibility:getComputedStyle(title.querySelector('img')).visibility}});
  assert.equal(vaultPending.textVisible,true);assert.equal(vaultPending.artVisibility,'hidden');releases.get('back-to-the-vault-vstyle-v1.svg')();await page.waitForFunction(()=>{const title=document.getElementById('vault-title');return title.querySelector('img').naturalWidth>0&&!title.classList.contains('paint-pending')});
  const vaultReady=await page.locator('#vault-title').evaluate(title=>{const r=title.getBoundingClientRect();return {x:r.x,y:r.y,width:r.width,height:r.height}});for(const key of ['x','y','width','height'])assert(Math.abs(vaultPending.rect[key]-vaultReady[key])<1,'vault title '+key+' shifted after SVG decode');
  await page.evaluate(()=>{document.getElementById('vault-title').scrollIntoView({block:'center'});scrollBy(0,80)});await page.waitForFunction(()=>document.getElementById('vault-title').dataset.paintProgress==='1.000');
  const vaultRevealed=await page.locator('#vault-title').evaluate(title=>{const r=title.getBoundingClientRect(),art=title.querySelector('img'),s=getComputedStyle(art);return {rect:{x:r.x,y:r.y,width:r.width,height:r.height},artVisible:s.visibility!=='hidden'&&s.display!=='none'&&r.bottom>0&&r.top<innerHeight}});assert.equal(vaultRevealed.artVisible,true,'completed vault title is not readable in the phone viewport');
  await page.screenshot({path:path.join(out,'chromium-phone-vault-title-revealed.png')});assert.deepEqual(errors,[]);results.push({case:'slow-title-pending-fallback',pending,complete,vaultPending,vaultReady,vaultRevealed,result:'PASS'});
 }finally{for(const release of releases.values())release();await context.close();await browser.close()}
}
