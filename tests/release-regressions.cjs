...
async function pendingTitle(){
 ...
 try{
  ...
  await page.screenshot({path:path.join(out,'chromium-phone-vault-title-ready.png')});assert.deepEqual(errors,[]);results.push({case:'slow-title-pending-fallback',pending,complete,vaultPending,vaultReady,result:'PASS'});
