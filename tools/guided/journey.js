...
const next=$('worldNext'),play=$('worldPlay'),video=$('worldVideo'),portrait=$('worldPortrait'),tryFlight=$('worldTryFlight');
...
function storyArrival(source,rect){
 ...
 animateMoment(portrait,[{opacity:0,clipPath:'inset(0 48% 0 48%)'},{opacity:1,clipPath:'inset(0 0% 0 0%)'}],{duration:1750,delay:180,fill:'backwards',easing:'cubic-bezier(.25,.7,.2,1)'});
 animateMoment($('portrait-title'),[{opacity:0,clipPath:'inset(0 100% 0 0)'},{opacity:1,clipPath:'inset(0 0% 0 0)'}],{duration:1350,delay:600,fill:'backwards',easing:'cubic-bezier(.22,.61,.36,1)'});
}
...
world.addEventListener('scroll',queueFlow,{passive:true});
...
function prepareFilm(){if(!video.getAttribute('src')){video.preload='metadata';video.src='assets/story-film-light.mp4'}}
function pauseFilm(){++filmToken;clearTimeout(filmTimer);filmStarting=false;video.pause()}
function failFilm(message){filmFailed=true;pauseFilm();if(video.error||video.readyState<2){filmRevealed=false;portrait.classList.remove('media-ready')}status(message)}
function bufferLimit(){clearTimeout(filmTimer);filmTimer=setTimeout(()=>{if(filmAllowed())failFilm('Film paused while loading. Scroll down to continue.')},6500)}
async function startFilm(){
 ...
 }catch(error){if(token!==filmToken)return;filmStarting=false;if(error.name==='NotAllowedError'){filmUserPaused=true;pauseFilm();status('Film stays still. Scroll down to continue.')}else failFilm('Film unavailable. Scroll down to continue.')}
}
function syncFilm(){if(filmAllowed())startFilm();else pauseFilm()}
video.addEventListener('playing',()=>{if(!filmAllowed()){video.pause();return;}filmStarting=false;const token=filmToken;
 const show=()=>{if(token!==filmToken||!filmAllowed())return;filmRevealed=true;portrait.classList.add('media-ready');clearTimeout(filmTimer);status('')};
 if(video.requestVideoFrameCallback)video.requestVideoFrameCallback(show);else show();
});
video.addEventListener('waiting',()=>{if(filmAllowed()){status('Film buffering. You can continue.');bufferLimit()}});
video.addEventListener('error',()=>{if(state==='story')failFilm('Film unavailable. Scroll down to continue.')});
...
next.addEventListener('click',()=>navigate(game));play.addEventListener('click',startGame);$('worldArrow').addEventListener('click',startGame);
...
new MutationObserver(()=>{if(document.body.classList.contains('next-vault-open')||document.body.classList.contains('next-drop-landed'))warm()}).observe(document.body,{attributes:true,attributeFilter:['class']});
